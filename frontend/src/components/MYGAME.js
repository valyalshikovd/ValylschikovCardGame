...
const MYGAME = ({socket, name, room}) => {
    ...
    class Card3 {
        ...
        const [table, setTable] = useState([(new Card3(0, 0))])
        useEffect(() => {
            ...
            socket.current.on("getOppsCards", (playerInfo1, room) => {
                ...
                const arr = []
                jsonObj.hand.forEach(function (element, index) {
                    arr.push(new Card3(jsonObj.hand[index].suit + jsonObj.hand[index].rank, jsonObj.hand[index].suit))
                });
                setOppsCards(arr)
            });
            ...
            socket.current.on("table", (json) => {
                ...
                setTable([new Card3(jsonObj.suit + jsonObj.rank, jsonObj.suit)])
